...
public class ItemDaoImpl extends HibernateDaoSupport implements ItemDao {
		...
		if (queryString != null && !"".equals(queryString)) {
			itemList = getHibernateTemplate().executeFind(new HibernateCallback() {
				...
				public Object doInHibernate(Session session) throws HibernateException, SQLException {
					return session
							.createQuery("from Item i where i.itemNo like ? or i.itemName like ? order by i.itemNo")
			...
			itemList = getHibernateTemplate().executeFind(new HibernateCallback() {
				...
				public Object doInHibernate(Session session) throws HibernateException, SQLException {
					return session
							.createQuery(
									"from Item i join fetch i.itemCategory join fetch i.itemUnit order by i.itemNo")
							.setFirstResult((pageNo - 1) * pageSize).setMaxResults(pageSize).list();
				}
			});
		}
	...
	private int getTotalRecords(String queryString) {
		...
		// if (queryString != null && !"".equals(queryString)) {
		// list = getHibernateTemplate().find("select count(*) from Item i where
		// i.itemNo like ? i.itemName like ?",
		...
		if (queryString != null && !"".equals(queryString)) {
			count = ((Long) getSession()
					.createQuery("select count(*) from Item i where i.itemNo like ? or i.itemName like ?")
					.setParameter(0, queryString + "%").setParameter(1, queryString + "%").uniqueResult()).intValue();
		} else {
			count = ((Long) getSession().createQuery("select count(*) from Item i").uniqueResult()).intValue();
		}
		return count;
	}
	...
	public void deleteItem(String[] itemNos) {
		for (int i = 0; i < itemNos.length; i++) {
			getHibernateTemplate().delete(getHibernateTemplate().load(Item.class, itemNos[i]));
		}
	}
}
